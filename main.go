...
func init() {
	...
	flag.BoolVar(&h3, "h3", false, "enable HTTP3 connections")
...
func visit(url *url.URL) {
	...
	case "https":
		...
		// Apply TLS config to either transport
		if h3Transport, ok := tr.(*http3.Transport); ok {
			h3Transport.TLSClientConfig = &tls.Config{
				ServerName:         host,
				InsecureSkipVerify: insecure,
				Certificates:       readClientCert(clientCertFile),
			}
		} else if httpTransport, ok := tr.(*http.Transport); ok {
	...
	printf("\n%s %s, %s\n", color.GreenString("Connected via"), color.CyanString("%s", connectedVia), color.CyanString("%s", protocol))
